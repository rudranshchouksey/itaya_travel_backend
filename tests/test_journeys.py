...
@pytest.mark.asyncio
async def test_journey_1_anonymous_discovery(async_client: AsyncClient, db_session: AsyncSession):
    ...
    # 1. Search listing
    res = await async_client.get(f"{settings.API_V1_PREFIX}/search", params={"query": "Journey"})
    ...
    res = await async_client.get(f"{settings.API_V1_PREFIX}/listings", params={"destination_id": dest_id})
    assert res.status_code == 200
    listings = res.json()
    assert len(listings) > 0
...
async def test_journey_2_authenticated_trip_builder(
    ...
    res = await async_client.post(f"{settings.API_V1_PREFIX}/auth/register", json=register_data)
    assert res.status_code == 201
    
    # 2. Login
    login_data = {
        "username": "journey2@example.com",
        "password": "StrongPassword123!"
    }
    res = await async_client.post(f"{settings.API_V1_PREFIX}/auth/login", data=login_data)
    assert res.status_code == 200
    ...
    res = await async_client.post(f"{settings.API_V1_PREFIX}/trips", json=trip_data, headers=headers)
    assert res.status_code == 201
    trip_id = res.json()["id"]
    ...
    res = await async_client.post(f"{settings.API_V1_PREFIX}/trips/{trip_id}/items", json=stay_item, headers=headers)
    assert res.status_code == 201
    ...
    res = await async_client.post(f"{settings.API_V1_PREFIX}/trips/{trip_id}/items", json=exp_item, headers=headers)
    assert res.status_code == 201
    
    # 6. Update trip
    res = await async_client.patch(f"{settings.API_V1_PREFIX}/trips/{trip_id}", json={"budget": 1500.00}, headers=headers)
    assert res.status_code == 200
...
async def test_journey_3_booking_and_payment_flow(
    ...
    res = await async_client.get(f"{settings.API_V1_PREFIX}/listings/{listing.id}/availability", params={
        "start_date": start_d.isoformat(),
        "end_date": end_d.isoformat()
    })
    assert res.status_code == 200
    ...
    headers = {"Authorization": f"Bearer {test_user_token}", "X-Payment-Token": "success_token"}
    res = await async_client.post(f"{settings.API_V1_PREFIX}/bookings", json=booking_req, headers=headers)
    assert res.status_code == 201
    booking_id = res.json()["id"]
    
    # 4. Retrieve booking
    res = await async_client.get(f"{settings.API_V1_PREFIX}/bookings/{booking_id}", headers=headers)
    assert res.status_code == 200
...
async def test_journey_4_review_flow(
    ...
    # 2. Retrieve review
    res = await async_client.get(f"{settings.API_V1_PREFIX}/listings/{listing.id}/reviews")
    assert res.status_code == 200
...
async def test_journey_5_ai_assisted_planning(
    ...
    res = await async_client.post(f"{settings.API_V1_PREFIX}/ai/trips/plan", json=ai_req)
    assert res.status_code == 200
    plan = res.json()
