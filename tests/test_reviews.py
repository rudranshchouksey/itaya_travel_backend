...
async def setup_review_data(db_session: AsyncSession, test_user: User):
    dest = Destination(
        name="Review Dest", slug=f"rev-dest-{uuid.uuid4().hex[:8]}", country="TC"
    )
    db_session.add(dest)
...
async def test_valid_review(
    ...
    (
        ...
        exp_item_id,
        pending_item_id,
    ) = await setup_review_data(db_session, test_user)
...
async def test_review_without_completed_booking(
    async_client: AsyncClient,
    test_user_token: str,
    db_session: AsyncSession,
    test_user: User,
...
async def test_unauthorized_review(
...
async def test_review_retrieval(
    async_client: AsyncClient,
    test_user_token: str,
    db_session: AsyncSession,
    test_user: User,
    ...
    review_data = {"booking_item_id": str(stay_item_id), "rating": 5, "title": "Nice!"}
    await async_client.post(
        f"{settings.API_V1_PREFIX}/reviews", json=review_data, headers=headers
    )
    ...
    await async_client.post(
        f"{settings.API_V1_PREFIX}/reviews", json=review_data_exp, headers=headers
    )

    # Get listing reviews
    res_list = await async_client.get(
        f"{settings.API_V1_PREFIX}/listings/{listing_id}/reviews"
    )
    ...
    res_exp = await async_client.get(
        f"{settings.API_V1_PREFIX}/experiences/{exp_id}/reviews"
    )
